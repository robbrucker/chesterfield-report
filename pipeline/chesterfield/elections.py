...
def _lookup_section() -> str:
    ...
    js = (
        ...
        "function portalErr(msg){o.innerHTML='<div class=\\\"el-res el-res-err\\\">'+msg+' Look up your address at <a href=\\\"'+PORTAL+'\\\" target=\\\"_blank\\\" rel=\\\"noopener\\\">the official Virginia portal</a>.</div>';}"
        "function fail(){portalErr('Something went wrong.');}"
        "function notFound(){portalErr('We could not match that address. Try adding your city and ZIP code, or');}"
        "function render(g,addr){"
        "var co=((g&&g['Counties'])||[])[0]||{};"
        ...
        "+'<div class=\\\"el-res-sub\\\">'+esc(addr)+'</div>'"
        ...
        "}"
        "var seq=0;"
        # generic JSONP loader: build(cb) returns the URL; onData(result) gets data or null
        "function jsonp(build,onData){var cb='__elcb'+(++seq),s=document.createElement('script'),to,fired=false;"
        ...
        "window[cb]=function(d){if(fired)return;fired=true;cleanup();onData(d);};"
        "s.onerror=function(){if(fired)return;fired=true;cleanup();onData(null);};"
        "to=setTimeout(function(){if(fired)return;fired=true;cleanup();onData(null);},20000);"
        "s.src=build(cb);document.body.appendChild(s);}"
        "function censusOne(a){return function(cb){return 'https://geocoding.geo.census.gov/geocoder/geographies/onelineaddress?address='+encodeURIComponent(a)+'&benchmark=Public_AR_Current&vintage=Current_Current&layers=all&format=jsonp&callback='+cb;};}"
        "function censusXY(lat,lon){return function(cb){return 'https://geocoding.geo.census.gov/geocoder/geographies/coordinates?x='+encodeURIComponent(lon)+'&y='+encodeURIComponent(lat)+'&benchmark=Public_AR_Current&vintage=Current_Current&layers=all&format=jsonp&callback='+cb;};}"
        "function nominatim(a){return function(cb){return 'https://nominatim.openstreetmap.org/search?q='+encodeURIComponent(a)+'&countrycodes=us&format=json&limit=1&json_callback='+cb;};}"
        # Step 1: Census text match. Step 2 (on miss): Nominatim -> coordinates -> Census-by-point.
        "function lookup(a){"
        "jsonp(censusOne(a),function(d){"
        "var m=d&&d.result&&d.result.addressMatches;"
        "if(m&&m.length){try{render(m[0].geographies,m[0].matchedAddress||a);}catch(_){fail();}return;}"
        "jsonp(nominatim(a),function(nd){"
        "if(!nd||!nd.length||!nd[0].lat){notFound();return;}"
        "var r=nd[0];"
        "jsonp(censusXY(r.lat,r.lon),function(cd){"
        "var g=cd&&cd.result&&cd.result.geographies;"
        "if(!g){notFound();return;}"
        "try{render(g,r.display_name||a);}catch(_){fail();}"
        "});});});}"
        "f.addEventListener('submit',function(e){e.preventDefault();var a=i.value.trim();if(!a){return;}"
        "o.innerHTML='<p class=\\\"el-loading\\\">Looking up your address...</p>';lookup(a);"
        "});})();</script>"
    )
    return (
